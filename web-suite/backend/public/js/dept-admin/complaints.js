...
window.assignOfficer = async function(complaintId) {
    ...
    try {
        ...
        officersData.data.officers.forEach(officer => {
            ...
            option.textContent = `${officer.officer_name} (${officer.badge_number})`;
...
function renderComplaints(complaints) {
    ...
    tbody.innerHTML = complaints.map(complaint => {
        ...
        <tr>
            ...
            <td><span class="badge badge-${getStatusClass(complaint.status)}">${complaint.status}</span></td>
            <td>${complaint.officer_name || 'Unassigned'}</td>
...
async function confirmAssignOfficer() {
    ...
    try {
        ...
        alert('Failed to assign officer: ' + error.message);
    }
}
